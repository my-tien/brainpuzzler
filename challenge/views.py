...
from jobs.forms import UploadFileForm
...
def challenge_upload(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            handle_upload(request.FILES['challenge_file'])
            return HttpResponseRedirect('/challenge/uploaded/')
        else:
            form = UploadFileForm()
        return render_to_response('k_challenge.html', {'form': form})


def handle_upload(file):
    with open('/srv/http/knossos-challenge/test.txt', 'wb+') as destination:
